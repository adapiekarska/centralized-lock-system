...
class CardLock:
    def __init__(self):
        self.table_name = table_names.CARDS_LOCKS_TABLE
        self.QUERY_CREATE = f'INSERT into {self.table_name} (CARD_ID, LOCK_ID) values (:card_id, :lock_id)'
    ...
    def create(self, card_id, lock_id):
        conn = create_connection('acs.db')
        with conn:
            return conn.execute(self.QUERY_CREATE, {"card_id": card_id, "lock_id": lock_id})
