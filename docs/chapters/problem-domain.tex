...
\label{chap:problem-domain}
	...
	\newsection{Podstawowe pojęcia}{Adrianna Piekarska}
		...
		W ramach niniejszej pracy termin \textit{identyfikator} stosowany jest jako określenie obiektu zawierającego układ RFID. Służy on poświadczeniu tożsamości podmiotów.
	...
	\newsection{Przegląd istniejących rozwiązań}{Grzegorz Wąs}
		...
		\begin{enumerate}[label=\Alph*.]
			\item System zamków mechanicznych
				...
				W tego typu systemie zamek zawiera układ mechaniczny otwierany przez fizyczny klucz. Jest to wciąż najczęściej stosowana metoda ochrony przestrzeni mieszkalnych, mimo że złamanie takiego zabezpieczenia wymaga jedynie odpowiednich umiejętności manualnych i zestawu narzędzi.
			...
			\item System oparty na komunikacji bezprzewodowej
	...
	\newsection{Bezpieczeństwo}{Grzegorz Wąs}

		Bezpieczeństwo (rozumiane jako ang. \textit{security} - połączenie cech poufności, integralności i dostępności, w przeciwieństwie do ang. \textit{safety} - nieobecność negatywnych konswekwencji dla użytkownika i środowiska \cite{security-safety}) jest jednym z głównych wyznaczników jakości systemu kontroli dostępu. Wielu producentów systemów elektronicznych nie podejmuje kroków w kierunku zapewnienienia odpowiedniego poziomu bezpieczeństwa swoich produktów, co często prowadzi do narażenia konsumentów na straty. Aby zminimalizować ryzyko strat, należy poświęcić odpowiednią uwagę zagadnieniom związanym z potencjalymi zagrożeniami i możliwymi słabymi punktami architektury. Rozproszone systemy elektroniczne są szczególnym wyzwaniem z punktu widzenia bezpieczeństwa. Poniżej przedstawiono wybrane punkty zaproponowanej w \cite{iot-vulnerabilities} klasyfikacji potencjalnych podatności w systemach IoT (ang. \textit{Internet of Things}, Internet Rzeczy):
